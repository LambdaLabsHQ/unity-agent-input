...
namespace LambdaLabs.UnityAgentInput
{
    ...
    ///
    /// Why this ability exists (tricks not discoverable from Unity docs):
    ...
    /// <code>
    /// using UnityEngine.InputSystem;
    /// LambdaLabs.UnityAgentInput.InputAbility.PressKey(Key.W);   // character starts moving forward
    /// // (advance frames via EditorApplication.Step / play_for_frames)
    /// LambdaLabs.UnityAgentInput.InputAbility.ReleaseKey(Key.W); // character stops
    /// </code>
    /// </summary>
    public static class InputAbility
    {
        ...
        /// <see cref="LambdaLabs.UnityAgentVision.GameViewCaptureAbility"/> output). Internally Y-flips to
        ...
        {
            // Clear held state immediately so ReapplySyntheticState won't re-inject.
            s_heldKeys.Clear();
            s_heldMouseButtons.Clear();

            // Schedule a one-shot onAfterUpdate handler that fires AFTER the current
            // frame's ReapplySyntheticState (if any), then force-clears device state.
            // This is necessary because EditorApplication.update (where REPL runs) fires
            // before onAfterUpdate, so InputState.Change called here gets overwritten by
            // the synthetic re-injection that happens later in the same frame.
